...
import { CanvasStub, createRealCanvas } from './CanvasStub';
import 'jest-canvas-mock';

const realCanvas = createRealCanvas();
if (realCanvas) {
  testSnakeGame(realCanvas, 'Real Canvas');
} else {
  throw new Error('Count not create Canvas');
}

const canvasStub = new CanvasStub();
testSnakeGame(canvasStub, 'ICanvas Stub');
...
function testSnakeGame(canvas: ICanvas, canvasType: String) {
  describe('test SnakeGame for ' + canvasType, () => {
